...
def fig_labels(clini: pd.DataFrame) -> None:
    ...
    def bars(ax, counts, order, title, colors):
        ...
        for xi, v in zip(x, vals):
            ax.text(
                xi, v + max(vals) * 0.02, str(v), ha="center", va="bottom", fontsize=10
            )

    bars(
        axes[0],
        clini["RESPONSE_CR"].value_counts(),
        ["yes", "no"],
        "RESPONSE_CR",
        [GREEN, GREY],
    )
    ...
    bars(
        axes[2],
        clini["BLAST_SEVERITY"].value_counts(),
    ...
    fig.suptitle(
        f"Clinical label distribution (n = {len(clini)} samples)",
        fontsize=13,
        color="#222",
        fontweight="bold",
        y=1.02,
...
def fig_cohort_cv(n_pat: int, n_samp: int, n_slide: int) -> None:
    fig, (axL, axR) = plt.subplots(
        1, 2, figsize=(13.3, 3.6), gridspec_kw={"width_ratios": [1, 1.25]}
    )
    ...
    axR.add_patch(
        plt.Rectangle((0.2, -0.55), 0.4, 0.32, facecolor=STEEL, edgecolor="white")
    )
    axR.text(0.72, -0.39, "train", va="center", fontsize=9)
    axR.add_patch(
        plt.Rectangle((2.0, -0.55), 0.4, 0.32, facecolor=AMBER, edgecolor="white")
    )
    axR.text(
        2.52,
        -0.39,
        "held-out test (every sample predicted once)",
        va="center",
        fontsize=9,
    )
...
def fig_workflow() -> None:
    ...
    ax.text(
        centers[0],
        2.65,
        "Weakly supervised: one label per slide, no cell annotations",
        ha="left",
        fontsize=10,
        style="italic",
        color="#555",
...
def select_candidates(clini: pd.DataFrame) -> dict:
    ...
    ss["top_enrichment_proxy_mean"] = pd.to_numeric(
        ss["top_enrichment_proxy_mean"], errors="coerce"
    )
    ss["blast_percent_gt"] = pd.to_numeric(ss["blast_percent_gt"], errors="coerce")
    ss["blast_percent_abs_err"] = pd.to_numeric(
        ss["blast_percent_abs_err"], errors="coerce"
    )
    ...
    def row_to_cand(r, note):
        ...
        return {
            ...
            "blast_pct_gt": None
            if pd.isna(r["blast_percent_gt"])
            else round(float(r["blast_percent_gt"]), 1),
            "enrichment": None
            if pd.isna(r["top_enrichment_proxy_mean"])
            else round(float(r["top_enrichment_proxy_mean"]), 3),
    ...
    cand = hb[
        (hb.high_blast_gt == "yes")
        & (hb.high_blast_pred_label == "yes")
        & (hb.flag_high_background == 0)
        & (hb.technical_concern_score <= 1)
    ]
    cand = cand.sort_values("top_enrichment_proxy_mean", ascending=False).head(8)
    out["high_blast"] = [
        c
        for c in (row_to_cand(r, "GT high / pred high") for _, r in cand.iterrows())
        if c
    ]
    ...
    lo = (
        good[good.blast_percent_gt < 5]
        .sort_values("top_enrichment_proxy_mean", ascending=False)
        .head(3)
    )
    hi = (
        good[good.blast_percent_gt >= 20]
        .sort_values("top_enrichment_proxy_mean", ascending=False)
        .head(3)
    )
    fails = (
        bp[bp.is_blast_percent_top10_failure == 1]
        .sort_values("blast_percent_gt", ascending=False)
        .head(3)
    )
    out["blast_percent"] = [
        c
        for c in (
            [
                row_to_cand(r, f"GT {r.blast_percent_gt:.0f}% (low) — correct")
                for _, r in lo.iterrows()
            ]
            + [
                row_to_cand(r, f"GT {r.blast_percent_gt:.0f}% (high) — correct")
                for _, r in hi.iterrows()
            ]
            + [
                row_to_cand(
                    r,
                    f"GT {r.blast_percent_gt:.0f}% -> pred {r.blast_percent_pred:.0f}% — FAILURE",
                )
    ...
    for sid in [c["sample_id"] for c in out["high_blast"][:5]]:
        ...
        if len(hbrow) and len(bprow):
            ...
            if hbc and bpc:
                pairs.append(
                    {"sample_id": sid, "high_blast": hbc, "blast_percent": bpc}
                )
        ...
        resp_rows.append(
            {
                ...
                "gt": pr["RESPONSE_CR"],
                "pred": pr["pred"],
                "p_yes": round(float(pr["RESPONSE_CR_yes"]), 3),
                "correct": bool(pr["RESPONSE_CR"] == pr["pred"]),
                "enrichment": round(float(cell.get(sid, np.nan)), 3)
                if sid in cell
                else None,
                "background": round(float(bg.get(sid, np.nan)), 3)
                if sid in bg
                else None,
                "overview": str(ov),
            }
...
def fig_explain_enrichment() -> None:
    ...
    ax.bar(
        ...
        hb,
        w,
        label="HIGH_BLAST model  (proxy AUROC 0.86)",
        color=NAVY,
        edgecolor="white",
    )
    ax.bar(
        ...
        bp,
        w,
        label="BLAST_PERCENT model  (proxy AUROC 0.42 ≈ chance)",
        color=GREY,
        edgecolor="white",
    )
    ...
    ax.spines[["top", "right"]].set_visible(False)
    ax.legend(
        frameon=False, loc="upper center", fontsize=10, bbox_to_anchor=(0.5, 1.02)
    )
    ax.annotate(
        "rises with blast burden",
        xy=(2 - w / 2, hb[2] + 0.005),
        xytext=(0.55, 0.20),
        fontsize=10,
        color=NAVY,
...
def compose_examples(task: str, picks: list[dict]) -> None:
    ...
    fig, axes = plt.subplots(
        2, n, figsize=(4.3 * n, 5.6), gridspec_kw={"height_ratios": [1, 1]}
    )
    ...
    for i, p in enumerate(picks):
        ...
        for row, img_path, sub in (
            (0, thumb, "WSI"),
            (1, overlay, "attention overlay"),
        ):
            ax = axes[row, i]
            if img_path.exists():
        ...
        axes[0, i].set_title(
            p["caption"], fontsize=11, color=NAVY, fontweight="bold", pad=6
        )
    fig.tight_layout()
...
def compose_all_examples(cands: dict) -> None:
    ...
    resp = [
        {
            **get("response", "SAMPLE_416_1012_19_A"),
            "kind": "classification",
            "cls": "no",
            "caption": "1012_19_A\nGT non-responder · pred non-responder",
        },
        {
            **get("response", "SAMPLE_380_1020_19_A"),
            "kind": "classification",
            "cls": "yes",
    ...
    bp = [
        {**get("blast_percent", sid), "kind": "regression", "caption": cap}

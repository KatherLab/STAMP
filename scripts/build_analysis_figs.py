...
NAVY, STEEL, GREY, AMBER, RED, GREEN = (
    "#1A4D8C",
    "#4E7CB5",
    "#8A94A6",
    "#C77F1A",
...
def compute_metrics(df: pd.DataFrame, task: str) -> dict:
    ...
    if KIND[task] == "multiclf":
        classes = [
            c.split(f"{label}_")[1] for c in df.columns if c.startswith(f"{label}_")
        ]
        aucs = {}
        for c in classes:
...
def fig_sample_vs_patient(metrics: dict) -> None:
    ...
    for off, key, color, name in [
        (-w / 2, "sample", GREY, "sample-split (leaky)"),
        (w / 2, "patient", NAVY, "patient-grouped (honest)"),
    ]:
        vals = [metrics[key][t]["auroc"] for t in tasks]
        err = [
            [
                metrics[key][t]["auroc"] - (metrics[key][t].get("lo") or np.nan)
                for t in tasks
            ],
            [
                (metrics[key][t].get("hi") or np.nan) - metrics[key][t]["auroc"]
                for t in tasks
            ],
        ]
        err = np.nan_to_num(
            np.abs(np.array(err)), nan=0.0
        )  # 0 = no bar (e.g. macro severity)
        ax.bar(
            x + off,
            vals,
            w,
    ...
    ax.set_title(
        f"Honest evaluation: patient-grouped vs sample-split CV\n"
...
def fig_auprc_calibration(new_preds: dict) -> None:
    ...
    for r, task in enumerate(tasks):
        ...
        ax.axhline(
            y.mean(), ls="--", color=GREY, lw=1, label=f"baseline {y.mean():.2f}"
        )
        ax.set_xlabel("recall")
        ax.set_ylabel("precision")
        ax.set_title(
            f"{NICE[task]} — PR (AP {ap:.2f})",
            color=NAVY,
            fontweight="bold",
            fontsize=11,
        )
        ...
        ax.set_title(
            f"{NICE[task]} — calibration", color=NAVY, fontweight="bold", fontsize=11
        )
        ax.set_xlim(0, 1)
        ax.set_ylim(0, 1)
        ax.spines[["top", "right"]].set_visible(False)
    fig.suptitle(
        "Patient-grouped models: precision-recall & calibration",
        fontsize=13,
        fontweight="bold",
        color="#222",
    )
...
def fig_response_beyond_blast(new_preds: dict, clini: pd.DataFrame) -> dict:
    ...
    bp = axL.boxplot(
        data, tick_labels=["responder", "non-responder"], patch_artist=True, widths=0.5
    )
    ...
    axL.text(
        2.4, 6, "5% (CR threshold)", color=RED, fontsize=9, ha="right", va="bottom"
    )
    axL.set_ylabel("clinical blast %")
    axL.set_title(
        f"Blast burden by response\n(point-biserial r = {corr:+.2f})",
        color=NAVY,
        fontweight="bold",
        fontsize=11,
    )
    ...
    axR.set_title(
        "Responder fraction by blast band", color=NAVY, fontweight="bold", fontsize=11
    )
    ...
    return {
        "blast_response_corr": corr,
        "resp_rate_by_band": resp_rate,
        "n_by_band": ns,
        "response_equals_lt5_blast": equiv,
        "strata_single_class": single,
    }
...
def _parse_protocol(fname: str) -> tuple[str, str]:
    ...
    defat = (
        "nicht_entfettet"
        if "nicht entfettet" in fname
        else ("entfettet" if "entfettet" in fname else "?")
    )
    return focus, defat
...
def fig_confounder(new_preds: dict, slide: pd.DataFrame) -> dict:
    ...
    axL.set_ylim(0, 1.05)
    axL.set_title(
        "Is scan protocol confounded with the label?",
        color=NAVY,
        fontweight="bold",
        fontsize=11,
    )
    ...
    bp = axR.boxplot(
        data,
        tick_labels=["5 Fokuspunkte", "10 Fokuspunkte"],
        patch_artist=True,
        widths=0.5,
    )
    ...
    axR.set_title(
        f"Does the score track scan protocol?\nAUROC(protocol ~ score) = {score_auc:.2f}",
        color=NAVY,
        fontweight="bold",
        fontsize=11,
    )
    ...
    return {
        "prop10_low": float(prop.get(0, 0)),
        "prop10_high": float(prop.get(1, 0)),
        "protocol_from_score_auroc": float(score_auc),
        "n": int(len(hb)),
    }
...
def main() -> None:
    ...
    for t in TASKS:
        ...
        if old is None or new is None:
            raise SystemExit(
                f"missing pooled preds for {t} (old={old is not None}, new={new is not None}) "
    ...
    (ASSETS / "metrics.json").write_text(
        json.dumps(metrics, indent=2, default=lambda o: None)
    )
    ...
    for t in TASKS:
        ...
        if KIND[t] == "reg":
            print(
                f"  {t:14s} Pearson r  sample {s['pearson']:.2f} -> patient {p['pearson']:.2f} "
                f"| R2 {s['r2']:.2f}->{p['r2']:.2f}"
            )
        else:
            print(
                f"  {t:14s} AUROC      sample {s['auroc']:.3f} -> patient {p['auroc']:.3f}"
            )

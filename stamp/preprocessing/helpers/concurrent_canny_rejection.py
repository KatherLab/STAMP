...
def reject_background(
    img: np.array, patch_size: Tuple[int, int], step: int, cores: int = 8
    ...
    with futures.ThreadPoolExecutor(cores) as executor:  # os.cpu_count()
        ...
        for i in i_range:
            for j in j_range:
                patch = img[
                    (i * patch_size[0]) : (i * patch_size[0] + step),
                    (j * patch_size[1]) : (j * patch_size[1] + step),
                ]
        ...
        ordered_patch_list = np.zeros(
            (x, patch_size[0], patch_size[1], 3), dtype=np.uint8
        )
    ...
    print(
        f"Finished Canny background rejection, rejected {np.sum(rejected_tile_list)} tiles: {end-begin:.2f} seconds"
    )

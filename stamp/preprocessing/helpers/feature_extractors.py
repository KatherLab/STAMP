...
from .swin_transformer import ConvStem, swin_tiny_patch4_window7_224
...
class FeatureExtractorCTP:
    ...
    def init_feat_extractor(self, device: str, **kwargs):
        ...
        self.model = swin_tiny_patch4_window7_224(
...
class FeatureExtractorUNI:
    def init_feat_extractor(self, device: str, **kwargs):
        ...
        model, transform = uni.get_encoder(
            enc_name="uni", device=device, assets_dir=asset_dir
        )
        self.model = model
        self.transform = transform

        digest = get_digest(
            f"{asset_dir}/vit_large_patch16_224.dinov2.uni_mass100k/pytorch_model.bin"
        )
...
def extract_features_(
    *,
    ...
    for batch in tqdm(dl, leave=False):
        feats.append(model(batch.type(dtype).to(device)).half().cpu().detach())

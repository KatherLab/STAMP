...
class JPGSlide:
    ...
    def get_thumbnail(self, dimensions):
        return cv2.resize(
            self.loaded_image, dsize=dimensions, interpolation=cv2.INTER_CUBIC
        )
...
class SlideReader:
        ...
        if not self.NotAbleToLoad:
            ...
            try:
                if ops.PROPERTY_NAME_MPP_X in self.slide.properties:
                    ...
                    self.MPP = (
                        1 / float(self.slide.properties["tiff.XResolution"]) * 10000
                    )
                else:
                    self.noMPPFlag = 1
    ...
    def build_generator(
        ...
        self.annPolys = [
            sg.Polygon(annotation.coordinates) for annotation in self.annotations
        ]
        ...
        def generator():
            for ci in range(len(self.coord)):
                ...
                filter_region = np.asarray(
                    self.slide.read_region(
                        c, self.slide.level_count - 1, [filter_px, filter_px]
                    )
                )[:, :, :-1]
                ...
                try:
                    region = np.asarray(
                        self.slide.read_region(c, 0, [self.extract_px, self.extract_px])
                    )[:, :, 0:3]
                    region = cv2.resize(
                        region, dsize=(size_px, size_px), interpolation=cv2.INTER_CUBIC
                    )
                except:
                    continue
                ...
                if exportFlag:
                    imageio.imwrite(
                        join(
                            tiles_path,
                            case_name + "_(" + str(c[0]) + "," + str(c[1]) + ").jpg",
                        ),
                        region,
                    )
                    if augment:
    ...
    def load_csv_roi(self, path):
        ...
        if "X_base" in headers and "Y_base" in headers:
            ...
            raise IndexError(
                'Unable to find "X_base" and "Y_base" columns in CSV file.'
            )
        self.annotations.append(AnnotationObject("Object" + str(len(self.annotations))))

        for index, row in reader.iterrows():
            if (
                str(row[index_x]).strip() == "X_base"
                or str(row[index_y]).strip() == "Y_base"
            ):
                self.annotations.append(
                    AnnotationObject(f"Object{len(self.annotations)}")
                )
                continue
...
class Convoluter:
    def __init__(
        self, size_px, size_um, stride_div, save_folder="", skipws=False, augment=False
    ):
    ...
    def convolute_slides(self):
        ...
        pool.map(
            lambda slide: self.export_tiles(self.SLIDES[slide], pb, ignoredFile_list),
            self.SLIDES,
        )
...
def get_args():
    ...
    parser.add_argument(
        "--skipws", type=bool, default=False, help="Shall we use whole slide images?"
    )
    parser.add_argument(
        "--px",
        type=int,
    ...
    parser.add_argument(
        "--num_threads", type=int, help="Number of threads to use when tessellating."
    )
...
if __name__ == ("__main__"):
    ...
    c = Convoluter(
        args.px, args.um, args.ov, args.out, augment=args.augment, skipws=args.skipws
    )
    ...
    outputFile.write(
        "Size of image patches to analyze, in pixels: " + str(args.px) + "\n"
    )
    outputFile.write(
        "Size of image patches to analyze, in microns: " + str(args.um) + "\n"
    )
    ...
    if isfile(args.slide):
        ...
        for root, dirs, files in os.walk(args.slide):
            for file in files:
                if (
                    ".ndpi" in file or ".scn" in file or "svs" in file or "tif" in file
                ) and not "csv" in file:

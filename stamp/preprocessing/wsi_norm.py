...
from .helpers.loading_slides import get_raw_tile_list, load_slide, process_slide_jpg
...
def save_image(image, path: Path):
    ...
    if width > 65500 or height > 65500:
        logging.warning(
            f"Image size ({width}x{height}) exceeds maximum size of 65500x65500, "
...
def preprocess(
    ...
    device: str = "cuda",
    normalization_template: Path = None,
    ...
    logfile_name = (
        "logfile_" + time.strftime("%Y-%m-%d_%H-%M-%S") + "_" + str(os.getpid())
    )
    ...
    logging.info(
        f"Norm: {norm} | Target_microns: {target_microns} | Patch_size: {patch_size} | MPP: {target_mpp}"
    )
    ...
    if has_gpu:
        print(
            f"Number of GPUs in the system: {torch.cuda.device_count()}, using device {device}"
        )
    ...
    existing = (
        [f.stem for f in output_file_dir.glob("**/*.h5")]
        if output_file_dir.exists()
        else []
    )
    if not only_feature_extraction:
        img_dir = [
            svs for ext in supported_extensions for svs in wsi_dir.glob(f"**/*{ext}")
        ]
        ...
        if not cache_dir.exists():
            logging.error(
                "Cache directory does not exist, cannot extract features from cached slides!"
            )
            exit(1)
    ...
    for slide_url in tqdm(
        img_dir, "\nPreprocessing progress", leave=False, miniters=1, mininterval=0
    ):
        slide_name = (
            slide_url.stem if not only_feature_extraction else slide_url.parent.name
        )
        ...
        if not (os.path.exists((f"{feat_out_dir}.h5"))) and not os.path.exists(
            f"{slide_url}.lock"
        ):
            with lock_file(slide_url):
                if (only_feature_extraction and (slide_jpg := slide_url).exists()) or (
                    slide_jpg := slide_cache_dir / "norm_slide.jpg"
                ).exists():
                    canny_norm_patch_list, coords_list, total = process_slide_jpg(
                        slide_jpg
                    )
                    print(
                        f"Loaded {img_name}, {len(canny_norm_patch_list)}/{total} tiles remain"
                    )
                else:
                    try:
                    ...
                    try:
                        slide_array = load_slide(
                            slide=slide, target_mpp=target_mpp, cores=cores
                        )
                    except MPPExtractionError:
                        if del_slide:
                            logging.error(
                                "MPP missing in slide metadata, deleting slide and continuing..."
                            )
                            if os.path.exists(slide_url):
                                os.remove(slide_url)
                        else:
                            logging.error(
                                "MPP missing in slide metadata, continuing..."
                            )
                        error_slides.append(slide_name)
                        continue
                    ...
                    bg_reject_array, rejected_tile_array, patch_shapes = (
                        reject_background(
                            img=slide_array,
                    ...
                    if norm:
                        ...
                        if cache:
                            save_image(
                                img_norm_wsi_jpg, slide_cache_dir / "norm_slide.jpg"
                            )
                    else:
                        canny_img, canny_norm_patch_list, coords_list = (
                            get_raw_tile_list(
                                slide_array.shape,
                                bg_reject_array,
                                rejected_tile_array,
                ...
                if len(canny_norm_patch_list) > 0:
                    extract_features_(
                        ...
                        norm_wsi_img=canny_norm_patch_list,
                        coords=coords_list,
                        wsi_name=slide_name,
                        outdir=feat_out_dir,
                        cores=cores,
                    ...
                    logging.info(
                        f"Extracted features from slide: {time.time() - start_time:.2f} seconds ({len(canny_norm_patch_list)} tiles)"
                    )
                    num_processed += 1
                else:
                    logging.error(
                        "0 tiles remain to extract features from after pre-processing. Continuing..."
                    )
                    error_slides.append(slide_name)
                    continue
        else:
    ...
    logging.info(
        f"===== End-to-end processing time of {num_total} slides: {str(timedelta(seconds=(time.time() - total_start_time)))} ====="
    )
    logging.info(
        f"Summary: Processed {num_processed} slides, encountered {len(error_slides)} errors, skipped {len(existing)} readily-processed slides"
    )
    if len(error_slides):
        logging.info(
            "The following slides were not processed due to errors:\n\n"
            + "\n".join(error_slides)
        )

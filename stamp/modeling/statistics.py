...
def add_roc_curve_args(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        required=True,
        type=Path,
        help=("Path to save the statistics to."),
    )
...
def compute_stats_(
    ...
    preds_dfs = [
        pd.read_csv(p, dtype={f"{target_label}": str, "pred": str}) for p in pred_csvs
    ]
    ...
    categorical_aggregated_(pred_csvs, target_label=target_label, outpath=stats_dir)

...
def main():
    ...
    parser.add_argument(
        "--cat_labels", type=str, nargs="+", default=[], help="Category labels"
    )
    parser.add_argument(
        "--cont_labels", type=str, nargs="+", default=[], help="Continuous labels"
    )
    parser.add_argument(
        "--categories", type=str, nargs="+", default=None, help="Categories"
    )
    ...
    group.add_argument(
        "--one_model",
        action="store_true",
        help="Run full training instead of cross-validation",
    )
    group.add_argument(
        "--deploy_model", type=Path, help="Path to the model .pkl to deploy"
    )
    ...
    if args.one_model:
        ...
        deploy_categorical_model_(
            ...
            slide_table=args.slide_table,
            feature_dir=args.feature_dir,
            checkpoint_path=args.deploy_model,
            output_dir=args.output_path,
            target_label=args.target_label,
            cat_labels=args.cat_labels,
            cont_labels=args.cont_labels,

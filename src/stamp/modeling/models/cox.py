"""
In parts from https://github.com/Novartis/torchsurv/blob/main/src/torchsurv/loss/cox.py
"""
...
def _partial_likelihood_cox(
    ...
    return (log_hz_sorted - log_denominator)[event_sorted.bool()]
...
def _partial_likelihood_efron(
    ...
    H = [
        torch.where((time_sorted == time_unique[j]) & (event_sorted.bool()))[0]
        for j in range(J)
    ]

...
class AttentionViewer:
    def __init__(
        ...
        # Initialize model
        self.model = LitVisionTransformer.load_from_checkpoint(checkpoint_path).to(device).eval()

        # Initialize napari viewer
        self.viewer = napari.Viewer(title=' Histopathology Attention Explorer')

        # Create dummy image layer
        self.image = np.zeros((100,100,3), dtype=np.float32)  # Placeholder for the image
        ...
        self.slide = None
        self.attention_map = np.zeros((100,100), dtype=np.float32)  # Placeholder for the attention map
    ...
    def _add_config_selection_ui(self):
        ...
        self.attention_handling.currentIndexChanged.connect(self._on_update_attention_map)
        ...
        self.layer_left_btn.clicked.connect(lambda: self._adjust_slider(value=-1, ui_element=self.layer_slider))
        ...
        self.layer_right_btn.clicked.connect(lambda: self._adjust_slider(value=1, ui_element=self.layer_slider))
        ...
        self.head_left_btn.clicked.connect(lambda: self._adjust_slider(-1, self.head_slider))
        ...
        self.head_right_btn.clicked.connect(lambda: self._adjust_slider(1, self.head_slider))
        ...
        self.topk_left_btn.clicked.connect(lambda: self._adjust_slider(-1, self.topk_slider))
        ...
        self.topk_right_btn.clicked.connect(lambda: self._adjust_slider(1, self.topk_slider))
    ...
    def _set_ui_enabled(self, enabled: bool):
        ...
        for btn_name in ['layer_left_btn', 'layer_right_btn', 'head_left_btn', 
                 'head_right_btn', 'topk_left_btn', 'topk_right_btn']:
    ...
    def process_selected_file(self, wsi_path):
        ...
        try: 
            ...
            with torch.no_grad():
                ...
                slide_mpp = get_slide_mpp_(self.slide, default_mpp=self.default_slide_mpp)
                ...
                self.attention_weights = self.model.vision_transformer.get_attention_maps(
                                        bags=feats.unsqueeze(0),
                                        coords=coords_um.unsqueeze(0),
                                        mask=torch.zeros(1, len(feats), dtype=torch.bool, device=self.device),
                                    )
    ...
    def _on_add_point(self):
        ...
        if not self.map_coords is None:
            ...
            if len(self.points_layer.data) > self._last_processed_point_count:  # If there's any data
                ...
                try:                
                    ...
                    x_snapped, y_snapped = self.map_coords[self.selected_token_idx,:].tolist()
                    self.points_layer.data = np.array([[y_snapped*8+4, x_snapped*8+4]])
                    ...
                    print(f"Clicked at coordinates: ({y},{x}). Selected token index: {self.selected_token_idx} at ({y_snapped*8+4},{x_snapped*8+4})")
                finally:
    ...
    def get_token_attention(self, selected_token_idx: IntType):
        ...
        if selected_direction == 0:
            token_attn = self.attention_map[selected_token_idx+1, 1:] # +1 to skip the cls token

        # Attention from others to selected
        elif selected_direction == 1:
            token_attn = self.attention_map[1:, selected_token_idx+1] # +1 to skip the cls token
        ...
        # Deviation of overall given attention
            ...
            token_attn = (token_attn - percentile_low) / (percentile_high - percentile_low + 1e-8)
    ...
    def _heatmap_generator(self, y: IntType, x: IntType):
        ...
        token_distances = cdist([(x, y)], self.map_coords.numpy(force=True)*8)  # Upscale by 8 to match thumbnail size
        ...
        cls_attn_map = cls_attn_map.repeat_interleave(8, dim=0).repeat_interleave(8, dim=1)

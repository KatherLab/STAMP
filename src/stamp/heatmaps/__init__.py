...
DEFAULT_SLIDE_MPP: SlideMPP = SlideMPP(1.0)
...
PATCH_SPACE_MODE = True

def _vals_to_im(
    scores: torch.Tensor,  # [N] or [N, C]
    coords_norm: torch.Tensor,  # ignored in patch-space
) -> torch.Tensor:
    """
    Unified value-to-image conversion.

    - Patch-space mode: layout by patch index (CPU, concrete sizes)
    - Grid mode: original coordinate-based layout
    """

    if PATCH_SPACE_MODE:
        # ---- PATCH-SPACE (robust, no geometry assumptions) ----
        scores_cpu = scores.detach().cpu()

        if scores_cpu.ndim == 1:
            scores_cpu = scores_cpu[:, None]

        N, C = scores_cpu.shape

        grid_w = int(np.ceil(np.sqrt(N)))
        grid_h = int(np.ceil(N / grid_w))

        im = np.zeros((grid_h, grid_w, C), dtype=np.float32)
        ...
        return torch.from_numpy(im).to(scores.device)

    # ------------------------------------------------------------------
    # ORIGINAL GRID MODE (unchanged, future-safe)
    # ------------------------------------------------------------------
    size = coords_norm.max(0).values.flip(0) + 1
    size = tuple(int(v.item()) for v in size)

    im = torch.zeros((*size, *scores.shape[1:]), device=scores.device)
    ...
    return flattened_im.reshape_as(im)


def vals_to_im_patchspace(
    scores: torch.Tensor,  # [N] or [N, C]
) -> np.ndarray:
    """
    Convert patch scores into a 2D visualization grid in patch-space.
    Always returns a concrete NumPy array.
    """
    ...
    # Arrange patches in a square-ish grid
    grid_w = int(np.ceil(np.sqrt(N)))
    grid_h = int(np.ceil(N / grid_w))
...
def heatmaps_(
    *,
    ...
    default_slide_mpp = (
        SlideMPP(default_slide_mpp)
        if default_slide_mpp is not None
        else DEFAULT_SLIDE_MPP
    )
    ...
    for wsi_path in wsis_to_process:
        ...
        with h5py.File(h5_path) as h5:
            ...
            feats = (
                torch.tensor(
                    h5["patch_embeddings"][:]  # pyright: ignore[reportIndexIssue]
                )
                .float()
                .to(device)
            )
            ...
            # Temporarily disable Microns type for stride to avoid issues with torch operations for multiplex data
            # stride_um = Microns(get_stride(coords_um))
            if coords_um.shape[0] < 2:
                # Single patch or degenerate case
                stride_um = Microns(coords_info.tile_size_um)
            elif (
                coords_um[:, 0].unique().numel() < 2
                or coords_um[:, 1].unique().numel() < 2
            ):
                # All patches in one row or column (ROI / component case)
                stride_um = Microns(coords_info.tile_size_um)
            else:
                # Regular grid → safe to infer stride
                stride_um = Microns(get_stride(coords_um))

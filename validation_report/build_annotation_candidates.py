...
EXP_BASE = {
    "response":       ("/mnt/nvme0n1p1/Jeff_projects/B01/AG Janssen/stamp_aml_response_uni2/crossval", "RESPONSE_CR", "RESPONSE_CR_yes", "classification"),
    "blast_percent":  ("/mnt/nvme0n1p1/Jeff_projects/B01/AG Janssen/stamp_aml_blast_percent_uni2/crossval", "BLAST_PERCENT", "pred", "regression"),
    "blast_severity": ("/mnt/nvme0n1p1/Jeff_projects/B01/AG Janssen/stamp_aml_blast_severity_uni2/crossval", "BLAST_SEVERITY", None, "classification"),
    "high_blast":     ("/mnt/nvme0n1p1/Jeff_projects/B01/AG Janssen/stamp_aml_high_blast_uni2/crossval", "HIGH_BLAST", "HIGH_BLAST_yes", "classification"),
}
...
def main():
    summary = {}
    sample_to_filename = {
        row["SAMPLE_ID"]: row["FILENAME"].replace(".h5", "")
    ...
    high_yes = df.nlargest(10, "RESPONSE_CR_yes")[["SAMPLE_ID", "stem", "RESPONSE_CR", "RESPONSE_CR_yes", "split"]]
    high_no  = df.nsmallest(10, "RESPONSE_CR_yes")[["SAMPLE_ID", "stem", "RESPONSE_CR", "RESPONSE_CR_yes", "split"]]
    boundary = df.iloc[(df["RESPONSE_CR_yes"] - 0.5).abs().argsort()[:10]][["SAMPLE_ID", "stem", "RESPONSE_CR", "RESPONSE_CR_yes", "split"]]
    ...
    high_pred = df.nlargest(10, "pred")[["SAMPLE_ID", "stem", "BLAST_PERCENT", "pred", "abs_err", "split"]]
    low_pred  = df.nsmallest(10, "pred")[["SAMPLE_ID", "stem", "BLAST_PERCENT", "pred", "abs_err", "split"]]
    big_errors = df.nlargest(10, "abs_err")[["SAMPLE_ID", "stem", "BLAST_PERCENT", "pred", "abs_err", "split"]]
    ...
    high_yes = df.nlargest(10, "HIGH_BLAST_yes")[["SAMPLE_ID", "stem", "HIGH_BLAST", "HIGH_BLAST_yes", "split"]]
    high_no  = df.nsmallest(10, "HIGH_BLAST_yes")[["SAMPLE_ID", "stem", "HIGH_BLAST", "HIGH_BLAST_yes", "split"]]
    boundary = df.iloc[(df["HIGH_BLAST_yes"] - 0.5).abs().argsort()[:10]][["SAMPLE_ID", "stem", "HIGH_BLAST", "HIGH_BLAST_yes", "split"]]
    ...
    cv_resp = cv_resp.merge(SLIDE_TABLE[["SAMPLE_ID", "FILENAME"]], on="SAMPLE_ID", how="left")
    ...
    confident_mistakes = cv_resp[(~cv_resp["correct"]) & (cv_resp["confidence"] > 0.7)] \
        .sort_values("confidence", ascending=False) \
        .head(15)[["SAMPLE_ID", "stem", "RESPONSE_CR", "pred", "RESPONSE_CR_yes", "confidence"]]
    summary["confident_mistakes_response"] = confident_mistakes.to_dict(orient="records")
    ...
    for r in summary["response"]["high_confidence_yes"][:5]:
        print(f"  {r['SAMPLE_ID']}  GT={r['RESPONSE_CR']}  pred_yes={r['RESPONSE_CR_yes']:.3f}  | {r['stem'][:55]}")
    ...
    for r in summary["confident_mistakes_response"][:5]:
        print(f"  {r['SAMPLE_ID']}  GT={r['RESPONSE_CR']}  pred={r['pred']}  conf={r['confidence']:.3f}  | {r['stem'][:55]}")
    print("\n=== BLAST_PERCENT largest errors (top 5) ===")
    for r in summary["blast_percent"]["largest_errors"][:5]:
        print(f"  {r['SAMPLE_ID']}  GT={r['BLAST_PERCENT']}%  pred={r['pred']:.1f}%  err={r['abs_err']:.1f}%  | {r['stem'][:55]}")
